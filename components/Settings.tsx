...
import { UserRole, RoomStatus, TaxCharge, Room, RatePlan, RoomType, BankAccount } from '../types.ts';
...
export const Settings: React.FC = () => {
    ...
    const handleAddBankAccount = () => {
        const newAccount: BankAccount = {
            ...
            accountNumber: '',
            accountName: '',
            isPrimary: localProp.bankAccounts.length === 0
        };
        setLocalProp({ ...localProp, bankAccounts: [...localProp.bankAccounts, newAccount] });
    };

    const handleUpdateBankAccount = (id: string, field: keyof BankAccount, value: string | boolean) => {
        const updated = localProp.bankAccounts.map(acc => acc.id === id ? { ...acc, [field]: value } : acc);
        setLocalProp({ ...localProp, bankAccounts: updated });
    };

    const handleRemoveBankAccount = (id: string) => {
        setLocalProp({ ...localProp, bankAccounts: localProp.bankAccounts.filter(acc => acc.id !== id) });
    };
    ...
    const BRAND_COLORS = [
        ...
        { id: 'violet', hex: '#8b5cf6', label: 'Nebula Violet' },
        { id: 'orange', hex: '#f97316', label: 'Radiant Orange' },
        { id: 'teal', hex: '#14b8a6', label: 'Aquatic Teal' },
        { id: 'fuchsia', hex: '#d946ef', label: 'Electric Fuchsia' },
        { id: 'lime', hex: '#84cc16', label: 'Cyber Lime' },
        { id: 'cyan', hex: '#06b6d4', label: 'Digital Cyan' }
    ];
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        <div className="lg:col-span-2 space-y-6">
                            <Card title="Corporate Identity Manifest">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div>
                                        <label className="block text-[10px] font-black uppercase text-slate-500 mb-2 ml-1">Communication Endpoint (Phone)</label>
                                    ...
                                    <div>
                                        <label className="block text-[10px] font-black uppercase text-slate-500 mb-2 ml-1">Digital Correspondence (Email)</label>
                                    ...
                                    <div className="md:col-span-2">
                                        <label className="block text-[10px] font-black uppercase text-slate-500 mb-2 ml-1">Physical Node Registry (Address)</label>
                            ...
                            <Card title="Fiscal Settlement Registry (Bank Accounts)">
                                <div className="space-y-6">
                                    <div className="flex justify-between items-center border-b pb-4 border-slate-100 dark:border-slate-800">
                                         <p className="text-[9px] font-black uppercase text-slate-400 tracking-widest">Active Settlement Nodes</p>
                                         <button 
                                            onClick={handleAddBankAccount}
                                            className="p-2 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-indigo-600 hover:text-white transition-all group border border-slate-200"
                                            title="Add New Account Node"
                                         >
                                    ...
                                    <div className="space-y-4">
                                        {localProp.bankAccounts.map((acc, idx) => (
                                            <div key={acc.id} className="p-6 bg-slate-50 dark:bg-slate-900 rounded-3xl border-2 border-slate-100 dark:border-slate-800 relative group animate-fade-in-right">
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                                    <div>
                                                        <label className="block text-[8px] font-black uppercase text-slate-400 mb-1">Bank Designation</label>
                                                        <input type="text" value={acc.bankName} onChange={e => handleUpdateBankAccount(acc.id, 'bankName', e.target.value)} className="w-full p-2 bg-white dark:bg-slate-800 border rounded-xl font-black uppercase text-[11px]" placeholder="e.g. Zenith Bank" />
                                                    </div>
                                                    <div>
                                                        <label className="block text-[8px] font-black uppercase text-slate-400 mb-1">Protocol Serial (A/C #)</label>
                                                        <input type="text" value={acc.accountNumber} onChange={e => handleUpdateBankAccount(acc.id, 'accountNumber', e.target.value)} className="w-full p-2 bg-white dark:bg-slate-800 border rounded-xl font-mono font-black text-[11px]" />
                                                    </div>
                                                    <div>
                                                        <label className="block text-[8px] font-black uppercase text-slate-400 mb-1">Account Beneficiary</label>
                                                        <input type="text" value={acc.accountName} onChange={e => handleUpdateBankAccount(acc.id, 'accountName', e.target.value)} className="w-full p-2 bg-white dark:bg-slate-800 border rounded-xl font-black uppercase text-[11px]" />
                                                    </div>
                                                </div>
                                            </div>
                                        ))}
                                        {localProp.bankAccounts.length === 0 && (
                                            <div className="py-10 text-center border-2 border-dashed border-slate-100 dark:border-slate-800 rounded-3xl">
                                                <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest">No Bank Nodes Registered</p>
                                                <button onClick={handleAddBankAccount} className="mt-4 text-indigo-600 text-[10px] font-black uppercase hover:underline">+ Initialize First Node</button>
                                            </div>
                                        )}
                                    </div>
                                </div>
                        ...
                        <div className="space-y-6">
                            <Card title="Visual Identity Protocol">
                                <p className="text-[9px] font-black uppercase text-slate-400 mb-4 tracking-widest">Interface Authority Color (Expanded)</p>
                                <div className="grid grid-cols-4 gap-3">
                                    {BRAND_COLORS.map(color => (
                                        <button
                                            key={color.id}
                                            onClick={() => setLocalProp({...localProp, brandColor: color.id})}
                                            className={`group relative h-10 rounded-xl border-2 transition-all ${localProp.brandColor === color.id ? 'border-slate-900 dark:border-white ring-2 ring-offset-2 dark:ring-offset-slate-950 scale-110' : 'border-transparent opacity-60 hover:opacity-100 hover:scale-110'}`}
                                            style={{ backgroundColor: color.hex }}
                                            title={color.label}
                                        >
                                            {localProp.brandColor === color.id && <div className="absolute inset-0 flex items-center justify-center bg-black/10"><svg className="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="4" d="M5 13l4 4L19 7"/></svg></div>}
                                        </button>
                                    ))}
                                </div>
                                <p className="text-[8px] font-black uppercase text-slate-500 mt-6 text-center tracking-widest italic opacity-60">Global UI Accent Matrix</p>
                            </Card>
